...
class Croutine : public CroutineImpl<Croutine<Args...>> {
    friend class CroutineImpl<Croutine<Args...>>;
...
class Croutine<> : public CroutineImpl<Croutine<>> {
    friend class CroutineImpl<Croutine<>>;
...
// _EngineCroutine is structurally identical to Croutine<>, for the engine's
// own internal use . The difference is _construct()/_destruct() are no-ops:
// engine-owned coroutines live inside global singletons that are never
// destroyed before late static destruction, so if they counted toward
// System's reference count the same way user coroutines do, System::quit() would
// never trigger early (via the last user resource going out of scope) it would
// always be deferred to late static destruction, which is exactly the unsafe timing
// this reference-counting mechanism exists to avoid.
class _EngineCroutine : public CroutineImpl<_EngineCroutine> {
    friend class CroutineImpl<_EngineCroutine>;
...
public:
    _EngineCroutine(void) = default;

    template<typename F>
    explicit _EngineCroutine(F&& function) : m_function(std::forward<F>(function)) {}

    _EngineCroutine(const _EngineCroutine&)            = delete;
    _EngineCroutine& operator=(const _EngineCroutine&) = delete;
    _EngineCroutine(_EngineCroutine&&)                 = delete;
    _EngineCroutine& operator=(_EngineCroutine&&)      = delete;
