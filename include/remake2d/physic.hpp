...
class PhysicManager {
private:
    // Box2D user data never points at user-owned PhysicBody objects (which can
    // move/realloc in the user's own containers). It points at a stable Slot
    // owned by the manager instead; the slot's `owner` pointer is kept in sync
    // whenever a PhysicBody moves, is copied, or is destroyed.
    struct Slot {
        PhysicBody* owner{nullptr};
        u64         id{0};
    };
    ...
    std::unordered_map<u64, std::unique_ptr<Slot>> m_slots;
...
private:
    ...
    void _rebindBody(PhysicBody* old_addr, PhysicBody* new_addr) noexcept;

    Slot* _slotFor(u64 id) noexcept;
    static PhysicBody* _ownerOf(void* shapeUserData) noexcept;
