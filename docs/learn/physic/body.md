...
they attach directly to the body, and are retrieved by name, which makes it easy to manage several on the same character .
so its position and rotation automatically follow the physics simulation, with no need to sync them by hand:
