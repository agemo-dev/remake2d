...
void quit(void);    // free all resources
bool isInit(void);  // check whether the engine has been initialized
```

Calling `init` is mandatory before using any module of **RE:MAKE 2D**. It becomes optional because it automatically called when engine need it , similary to `quit` .
