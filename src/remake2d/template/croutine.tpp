...
CroutineImpl<Derived>::CroutineImpl(void) {
    static_cast<Derived*>(this)->_construct();
}
...
CroutineImpl<Derived>::~CroutineImpl(void) {
    stop();
    static_cast<Derived*>(this)->_destruct();
}
...
void Croutine<Args...>::_construct(void) { rmk::system._init(); }

template<typename... Args>
void Croutine<Args...>::_destruct(void) { rmk::system._quit(); }
...
inline void Croutine<>::_construct(void) { rmk::system._init(); }
inline void Croutine<>::_destruct(void)  { rmk::system._quit(); }
...
template<typename F>
void _EngineCroutine::load(F&& newFunc) {
    m_function = std::forward<F>(newFunc);
}

inline void _EngineCroutine::run(void) {
    if (!m_function) throw CroutineError(error::croutine::any_function_linked);
    m_stopped = false;
    m_running = true;
    m_task.emplace(m_function());
